...
def main():
    ...
    if latitude1 is not None and longitude1 is not None and latitude2 is not None and longitude2 is not None:
        ...
        # Ajouter le texte avec la distance à la carte
        texte_distance = f"Distance à vol d'oiseau : {distance_vol_oiseau:.2f} km"
        folium.Marker([(latitude1 + latitude2) / 2, (longitude1 + longitude2) / 2], popup=texte_distance).add_to(carte)

        # Enregistrer la carte au format HTML
        carte.save("carte_villes_distance.html")
        print(f"La carte a été enregistrée sous le nom 'carte_villes_distance2.html'.")
    else:
